...
| `USE_ITEM_ON_BLOCK` | `useItemOn(Level, Player, InteractionHand, ItemStack, BlockState, BlockHitResult)` | `InteractionResult` |
| `USE_BLOCK_WITHOUT_ITEM` | `useWithoutItem(Level, Player, BlockState, BlockHitResult)` | `InteractionResult` |
| `USE_ITEM_ON` | `useOn(UseOnContext)` | `InteractionResult` |
| `USE_ITEM` | `use(Level, Player, InteractionHand)` | `InteractionResult` |
...
Returning a non-`PASS` result from any of these replaces the vanilla behaviour and is handed
back to the game in its place. All four fire on both the logical client and the logical server.
